...
<body class="bg-gradient-to-r from-teal-100 via-purple-100 to-pink-100 min-h-screen">
    <div class="container mx-auto py-10 px-4">
        <h1 class="text-4xl font-extrabold text-center text-indigo-900 mb-8 fade-in-up">Admin - Manage Articles</h1>

        <?php if (count($articles) > 0): ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach ($articles as $articleItem): ?>
                    <div class="bg-white rounded-lg shadow-xl hover:shadow-2xl transition-shadow duration-300 p-6 fade-in-up transform hover:-translate-y-2">
                        <h2 class="text-2xl font-bold text-indigo-800 mb-4"><?= htmlspecialchars($articleItem['title']) ?></h2>
                        <p class="text-gray-600 mb-4">
                            <?= htmlspecialchars(substr($articleItem['content'], 0, 100)) ?>
                        </p>
                        <p class="text-gray-500 text-sm mb-4">Status: 
                            <span class="font-semibold text-teal-600"><?= htmlspecialchars($articleItem['status']) ?></span>
                        </p>
        ...
        <div class="mt-8 text-center fade-in-up">
            <a href="../front-end/dashboard.php" 
               class="inline-block bg-red-600 text-white px-6 py-3 rounded-lg hover:bg-red-700 transform hover:scale-105 transition-transform duration-300 shadow-md">
                Back to Dashboard
            </a>
        </div>
    </div>
